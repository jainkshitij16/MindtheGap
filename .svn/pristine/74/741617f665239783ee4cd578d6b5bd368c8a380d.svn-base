...
public class TfLOverlayManager implements MapEventsReceiver {
    ...
    private StaticCluster staticCluster;
    private Station selectedStation;
    private boolean haveCleared;
    ...
    public void onStationMarkerLongPress(Station longPressedStn) {
        ...
        haveCleared = false;
        for (Line line : longPressedStn.getLines()) {
    ...
    public boolean longPressHelper(GeoPoint geoPoint) {
        ...
        haveCleared = true;
        for(Marker next: stnClusterer.getItems()){
            GeoPoint goPoint = next.getPosition();
            if(!goPoint.equals(geoPoint)){
                point = false;
            } else if(goPoint.equals(geoPoint)){
                point = true;
            }
        }
        if(!point){
    ...
    private void markStations() {
        ...
        for (Station next : StationManager.getInstance()) {
            ...
            staticCluster = new StaticCluster(geoPoint);
            stnClusterer.buildClusterMarker(staticCluster, mapView);
        }
    }
